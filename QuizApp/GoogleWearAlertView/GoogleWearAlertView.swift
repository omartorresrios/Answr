...
extension UIColor {
    class func successGreen() -> UIColor {
        return UIColor(red: 21.0/255.0, green: 216.0/255.0, blue: 161.0/255.0, alpha: 1)
    }
...
class GoogleWearAlertView: UIView, UIGestureRecognizerDelegate {
    ...
    init(title:String, image:UIImage?, type:GoogleWearAlertType, duration:Double, viewController:UIViewController, position:GoogleWearAlertPosition, canbeDismissedByUser:Bool) {
        ...
        titleLabel.frame = CGRect(x: self.center.x - (frame.size.width * titleLabelWidth) / 2, y: iconImageView.frame.origin.y + iconImageView.frame.size.height + 3, width: frame.size.width * titleLabelWidth, height: titleLabelHeight)
