...
class CommentViewController: UIViewController, UITextViewDelegate, UITextFieldDelegate, MFMailComposeViewControllerDelegate {
...
//            }
    ...
    @IBAction func addCommentAction(_ sender: AnyObject) {
        ...
        alertView.addButton("\(FIRAuth.auth()!.currentUser!.displayName!)") {
            ...
            if self.conditionalCounter < self.maxNumberComments {
                self.counter += 1
                self.selectedQuestion.ref.child("counterComments").setValue(self.counter)
    ...
    @IBAction func reportContent(_ sender: AnyObject) {
        let appearance = SCLAlertView.SCLAppearance(
